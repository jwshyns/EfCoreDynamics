...
public abstract class AsyncSimpleQueryTestBase<TFixture> : QueryTestBase<TFixture>
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    {
        List<IGrouping<Guid, Order>> groups;
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public virtual async Task Mixed_sync_async_query()
    {
        ...
        var results
            = (await context.Customers
                .Select(c => new
                    { c.CustomerID, Orders = context.Orders.Where(o => o.Customer.CustomerID == c.CustomerID) })
                .ToListAsync())
            .Select(x => new
            {
                Orders = x.Orders
                    .GroupJoin(
                        new[] { NorthwindData.AlfkiId }, y => x.CustomerID, y => y, (h, id) => new { h.Customer })
            })
            .ToList();
    ...
    protected virtual async Task Single_Predicate_Cancellation_test(CancellationToken cancellationToken)
    {
        ...
        var result = await ctx.Customers.SingleAsync(c => c.CustomerID == NorthwindData.AlfkiId, cancellationToken);

        Assert.Equal(NorthwindData.AlfkiId, result.CustomerID);
    }
    ...
    public virtual async Task Throws_on_concurrent_query_list()
    {
        ...
        var throwingTask = Task.Run(async () =>
        {
            ...
            Assert.Equal(
                ...
                (await Assert.ThrowsAsync<InvalidOperationException>(() => context.Customers.ToListAsync())).Message);
        });
    ...
    public virtual async Task Throws_on_concurrent_query_first()
    {
        ...
        var throwingTask = Task.Run(async () =>
        {
            ...
            Assert.Equal(
                CoreStrings.ConcurrentMethodInvocation,
                (await Assert.ThrowsAsync<InvalidOperationException>(() => context.Customers.FirstAsync())).Message);
        });
        ...
        var query = await context.Customers.OrderBy(c => c.CustomerID)
            .Select(c => new
            {
                ...
                Value = c.CustomerID == NorthwindData.AlfkiId | c.CustomerID == NorthwindData.AnatrId |
                        c.CustomerID == NorthwindData.AntonId
            })
            .ToListAsync();
    ...
    public virtual async Task Select_bitwise_and_or()
    {
        ...
        Assert.All(query.Where(c => c.CustomerID != NorthwindData.AntonId), t => Assert.False(t.Value));
    }
